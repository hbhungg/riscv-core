...
class Register:
  ...
  def hexfmt(self, key):
    return f"{self.regs[key]:08x}" if self.regs[key] != 0 else " " * 7 + "0"  # Format hex 08x

  def __repr__(self):
    return (
      "---- Register state ----\n"
      + "\n".join([" ".join([f"{REGISTERS_NAME[4 * i + j]}: {self.hexfmt(4 * i + j)}".rjust(16) for j in range(4)]) for i in range(8)])
      + f"\nPC: {self.hexfmt(32)}\n"
    )
...
class CPU:
  ...
  def load(self, addr, data):
    ...
    if addr < 0 or addr >= len(self.memory):
      raise InvalidMemory(f"Address {addr:08x} is out of bound for {len(self.memory):08x}")
  ...
  def step(self):
    ...
    if DEBUG > 1:
      print(f"[{self.register.hexfmt(32)}] Raw instruction: {bin(ins)} ({hex(ins)})")
    ...
    if opcode == Ops.JAL:
      if DEBUG > 0:
        print(self.register.hexfmt(32), opcode, REGISTERS_NAME[rd], hex(imm_j))
      ...
      if DEBUG > 0:
        print(self.register.hexfmt(32), opcode, REGISTERS_NAME[rd], REGISTERS_NAME[rs1], hex(imm_i))
      self.register[rd] = self.alu(funct3.ADD, 4, vpc, 0)  # Store the next instruction addr
      self.register[Register.PC] = self.alu(funct3.ADD, vs1, imm_i, alt)
      if DEBUG > 1:
        print(self.register, end="\n")
      return True  # HACK: REMOVE THIS
    elif opcode == Ops.BRANCH:
      if DEBUG > 0:
        print(self.register.hexfmt(32), opcode, funct3, REGISTERS_NAME[rs1], REGISTERS_NAME[rs2], hex(imm_b))
      ...
      if DEBUG > 0:
        print(self.register.hexfmt(32), opcode, funct3, REGISTERS_NAME[rd], REGISTERS_NAME[rs1], hex(imm_i), funct3, funct7)
      self.register[rd] = self.alu(funct3, vs1, imm_i, alt)
    elif opcode == Ops.AUIPC:
      if DEBUG > 0:
        print(self.register.hexfmt(32), opcode, REGISTERS_NAME[rd], hex(imm_u))
      ...
      if DEBUG > 0:
        print(self.register.hexfmt(32), opcode, funct3, REGISTERS_NAME[rd], REGISTERS_NAME[rs1], REGISTERS_NAME[rs2])
      ...
      if DEBUG > 0:
        print(self.register.hexfmt(32), opcode, funct3, REGISTERS_NAME[rs1], REGISTERS_NAME[rs2], hex(imm_s))
      ...
      if DEBUG > 2:
        self.coredump(vs1 + imm_s - 4, l=32)

    elif opcode == Ops.SYSTEM:
      if funct3 == Funct3.ECALL:
        if DEBUG > 0:
          print(self.register.hexfmt(32), opcode, "ECALL", REGISTERS_NAME[rd], REGISTERS_NAME[rs1])
        ...
        if funct3 == Funct3.CSRRW:
          if DEBUG > 0:
            print(self.register.hexfmt(32), opcode, funct3, imm_i)
        else:
          if DEBUG > 0:
            print(self.register.hexfmt(32), opcode, "SKIP", imm_i)
    elif opcode == Ops.MISC:
      if DEBUG > 0:
        print(self.register.hexfmt(32), opcode, "SKIP")
    else:
      if DEBUG > 0:
        print(self.register.hexfmt(32), opcode, REGISTERS_NAME[rd])
      raise NotImplementedError
